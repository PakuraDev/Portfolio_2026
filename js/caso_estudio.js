document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!projectId) {
    showError("No se ha especificado ningún proyecto. (Añade ?id=aerko a la URL)");
    return;
  }

  // 1. Cargar el JSON
  fetch('./casos_de_estudio/proyectos.json')
    .then(res => {
      if (!res.ok) throw new Error("No se pudo cargar la base de datos de proyectos.");
      return res.json();
    })
    .then(data => {
      if (!data[projectId]) throw new Error("El proyecto especificado no existe.");
  ...
  function renderProject(data) {
    ...
    document.getElementById('project-question').textContent = `¿Qué quieres saber de ${data.titulo}?`;
    ...
    data.enlaces.forEach(link => {
      ...
      // Mapeo simple de iconos (se podrían usar SVGs reales aquí)
      let iconSvg = '';
    ...
    // Seleccionar la pestaña por defecto o la que viene por URL
  ...
  // 3. Lógica de Pestañas (Selector + Ola)
  function selectTab(selectedOption, isInitialLoad = false) {
    ...
    fetch(`./casos_de_estudio/${fileName}`)
      .then(res => {
        if (!res.ok) throw new Error('Error cargando el archivo parcial');
        return res.text();
      })
      .then(html => {
        setTimeout(() => {
      ...
      .catch(err => {
        console.error(err);
        setTimeout(() => {
          partialContent.innerHTML = `<div style="padding: 96px 48px; min-height: 50vh;"><h2 class="text-h2 text-blanco" style="text-align: center;">Error al cargar la sección</h2></div>`;
          partialContent.style.transition = 'opacity 0.4s ease';
          partialContent.style.opacity = 1;
        }, 500);
      });
